document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadSong(index) {
        ...
        document.body.style.backgroundImage = `url('${song.banner}')`;
        audio.src = song.src;
        audio.load();
    }
    window.onload = function() {
        loadSong(0); // Load the first song (or whichever index you prefer)
    };
...
//
const closebtn = document.getElementsByClassName("playlist_title");

for (let i = 0; i < closebtn.length; i++) {
    closebtn[i].addEventListener("click", function () {
        for (let j = 0; j < playlist.length; j++) {
            const currentStyle = playlist[j].style.position;
            playlist[j].style.position = currentStyle !== 'absolute' ? 'absolute' : 'static';
        }
    });
}

// For Re-Load Confermation
